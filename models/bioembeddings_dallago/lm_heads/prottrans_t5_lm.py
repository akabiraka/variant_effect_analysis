...
class ProtTransT5LM(EmbedderWithFallback, abc.ABC):
    ...
    def __init__(self, **kwargs):
        ...
        self._decoder = self._options.get("decoder", True) #False
        # print(self._decoder)
    ...
    def get_model(self) -> T5ForConditionalGeneration:
        if self._half_precision_model:
            model = T5ForConditionalGeneration.from_pretrained(self._model_directory, torch_dtype=torch.float16)
        else:
            model = T5ForConditionalGeneration.from_pretrained(self._model_directory)
            
        return model 

    # def _get_fallback_model(self) -> Union[T5Model, T5EncoderModel]:
    def _get_fallback_model(self) -> T5ForConditionalGeneration:
    ...
    def _embed_batch_impl(
            self, batch: List[str], model: T5ForConditionalGeneration
...
class ProtTransT5UniRef50LM(ProtTransT5LM):
...
class ProtTransT5XLU50LM(ProtTransT5LM):
    """Encoder of the ProtTrans T5 model trained on BFD and finetuned on UniRef 50.
